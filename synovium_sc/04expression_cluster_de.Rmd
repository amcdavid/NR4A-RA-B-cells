...
Try scds for doublet detection
Class switch distributions by cluster
BCR (molecules) recovery by cluster
...
```{r mappingstat}
stats = list.files('deliv_Anolik_042318_cellRanger_only_filtered/', pattern = 'metrics_summary.csv', full.names = TRUE, recursive = TRUE) 
...
```{r}
gg = glm(seqgek_qc_ok ~ log10_total_counts_mito + log10_total_features, data = colData(all_res), family = 'binomial')
all_res$logistic_qc_ok = predict(gg)>0

plotColData(all_res, x = "log10_total_counts_mito", y = "log10_total_features", colour_by = 'logistic_qc_ok', alpha = .5)
```
...
```{r}
blacklist = rep(FALSE, nrow(sce))
```

```{r trendvar}
fit.g = scran::trendVar(sce, subset.row = !blacklist, use.spikes = FALSE, parametric = TRUE)
...
trendvar_tidy = rowData(sce) %>% as.data.frame %>% mutate(use = !blacklist, trend = fit.g$trend(mean), cut_mean = cut(sqrt(mean), 5)) %>% group_by(cut_mean, use) %>% mutate(rank_by_mean = rank(-bio/total))
...
split_and_cluster = function(ssce_, id, orig_markers, extra = character(), resolution_=0.5, top_n = 10){
...
ig_plot = ggplot(ig_coexp, aes(x = IGHD, y = AMmisc::clamp(IGHM, 100))) + geom_jitter(aes(color = class_heavy), width = .02, height = .02) + scale_x_continuous(trans = scales::log1p_trans()) + scale_y_continuous(trans = scales::log1p_trans()) + facet_wrap(~label, scales = 'free') + geom_smooth(method = 'lm')

ig_plot + ylab('IGHM')
ig_plot + aes(x = AMmisc::clamp(IGHG3, 100)) + ylab('IGHG3')
